...
import { useLocation2g } from '@renderer/model';
...
const LocationTable: FC<LocationTableProp> = () => {
    ...
    const { location2gData } = useLocation2g();
    const columns = getLocationColumns(location2gData);

    const renderData = (): any[] => {
        const fields: string[] = columns.map(col => col.dataIndex);//取所有的列名

        return location2gData.map(item => {
