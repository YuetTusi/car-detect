 ...
 */
function getDb<T = any>(collection: string): Db<T> {
...
class Db<T = any> {
   ...
   */
  findOne(condition: Record<string, any> | null | undefined): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      this._instance.loadDatabase((err) => {
        if (err) {
          reject(err);
   ...
   */
  count(condition: Record<string, any> | null | undefined): Promise<number> {
    return new Promise<number>((resolve, reject) => {
      this._instance.loadDatabase((err) => {
        if (err) {
          reject(err);
